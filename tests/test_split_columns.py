...
def test_split_columns():
    ...
    expected = spark.createDataFrame(
        data, ["graduation_year", "major", "student_first_name", "student_last_name"]
    )
...
def test_split_columns_advanced():
    data = [
        ("chrisXXsomethingXXmoe", 2025, "bio"),
        ("davidXXbb", 2026, "physics"),
        (None, 2025, "physics"),
    ]
    df = spark.createDataFrame(data, ["student_name", "graduation_year", "major"])
...
def test_split_columns_strict():
    ...
    with pytest.raises(
        Exception
    ):  # there is no way to make it work for all the versions
        df2.show()
