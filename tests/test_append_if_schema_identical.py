...
def test_append_if_schema_identical(spark):
    ...
    unidentical_df = spark.createDataFrame(bad_data, schema=StructType([
        StructField("id", IntegerType()),
        StructField("name", StringType()),
        StructField("city", StringType()),
    ...
    if not check_if_error_caught:
        raise AssertionError("append_if_schema_identical should raise an error if the schemas are not identical")
